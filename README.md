...
School Information and Management System for third party metadata storage.
...
```bash
conda create python=3.8 -n school_information_system
```
2 - <a name="step-2">Activate the conda environment</a>

```bash
conda activate school_information_system
```
...
If for some reason you get an error similar to bellow, is because the DATABASE_URL is configured to `postgres:///school_information_system` and because of it the generated `DATABASES` settings are configured to connect on PostgreSQL using the socket mode.
...
LINE 1: CREATE DATABASE school_information_system WITH ENCODING 'UTF-8' OWNER "";
                                                             ^
ERROR:  syntax error at or near ";"
LINE 1: GRANT ALL PRIVILEGES ON DATABASE school_information_system TO ;
```
...
ERROR:  database "school_information_system" already exists
GRANT
```
...
```bash
sudo -u postgres dropdb -U postgres --if-exists school_information_system
...
```bash
dropdb --if-exists school_information_system
